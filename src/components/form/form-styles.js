...
export default css`
  ...
  .tipo-item:has(input[value="Agua"]) {
    background-color: #bde0fe;
    color: #023e8a;
  }
  .tipo-item:has(input[value="Fuego"]) {
    background-color: #ffb3a7;
    color: #b30000;
  }
  .tipo-item:has(input[value="Planta"]) {
    background-color: #b9fbc0;
    color: #1b4332;
  }
  .tipo-item:has(input[value="Eléctrico"]) {
    background-color: #fff8b4;
    color: #8a6d00;
  }
  .tipo-item:has(input[value="Roca"]) {
    background-color: #d8c7a3;
    color: #4b371c;
  }
  .tipo-item:has(input[value="Tierra"]) {
    background-color: #e9d8a6;
    color: #5c3d00;
  }
  .tipo-item:has(input[value="Normal"]) {
    background-color: #f1f1f1;
    color: #444;
  }
  .tipo-item:has(input[value="Lucha"]) {
    background-color: #ffc6b6;
    color: #6b1b00;
  }
  .tipo-item:has(input[value="Siniestro"]) {
    background-color: #a8a8a8;
    color: #111;
  }
  .tipo-item:has(input[value="Acero"]) {
    background-color: #cfd8dc;
    color: #263238;
  }
  .tipo-item:has(input[value="Psíquico"]) {
    background-color: #ffb6d9;
    color: #5c1a5f;
  }
  .tipo-item:has(input[value="Fantasma"]) {
    background-color: #d0bdf4;
    color: #3b2066;
  }
  .tipo-item:has(input[value="Bicho"]) {
    background-color: #e1f8b0;
    color: #42590b;
  }
  .tipo-item:has(input[value="Veneno"]) {
    background-color: #e2b8ff;
    color: #4b0082;
  }
  .tipo-item:has(input[value="Volador"]) {
    background-color: #c7eaff;
    color: #023047;
  }
  .tipo-item:has(input[value="Hada"]) {
    background-color: #ffd6ec;
    color: #a020f0;
  }
  .tipo-item:has(input[value="Hielo"]) {
    background-color: #bdeff7;
    color: #005f73;
  }
  .tipo-item:has(input[value="Dragón"]) {
